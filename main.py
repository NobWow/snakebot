...
class PtDiscordBot(commands.Bot):
    ...
    def __init__(self, logger, conf, data, lang, *args, **kwargs):
        ...
        self.debug_prints = {"checks": False, "author_commands": True, "guild_owner_commands": True, "message_rendering": False, "unknown": False, "other": True}
        self.logger = logger
        self.conf = conf
        self.data = data
        self.mods = []
        ...
        def check_author_ban(ctx):
            ...
            try:
                if self.data['user_data'][str(ctx.author.id)]['author_ban']:
                    ...
                    self.debug_print('authorban check failed', 'checks')
                    return False
                else:
                    return True
        ...
        def check_author_ban_guild(ctx):
            ...
            try:
                if self.data['guild_data'][str(ctx.guild.id)]['author_ban']:
                    self.debug_print('yeah, this guild (ID %s, name %s) is banned' % (ctx.guild.id, ctx.guild.name) , 'checks')
                ...
                self.debug_print('Guild (ID %s, name %s) is not banned, check passed: %s' % (ctx.guild.id, ctx.guild.name, exc), 'checks')
                return True
        @self.check
        def check_guild_admin_ban_here(ctx):
            self.debug_print('check if the user (ID %s, name %s#%s) is banned on specific place by moderator.' % (ctx.author.id, ctx.author.name, ctx.author.discriminator), 'checks')
            if not ctx.guild:
            ...
            try:
                ...
                if str(ctx.author.id) in self.data['guild_data'][str(ctx.guild.id)]['placebanned_users'][str(ctx.channel.id)]:
                    self.debug_print('this user is banned on channel (ID %s, name %s) of the guild (ID %s, name %s)' % (ctx.channel.id, ctx.channel.name, ctx.guild.id, ctx.guild.name), 'checks')
        ...
        def check_guild_admin_block_category(ctx):
            ...
            try:
                ...
                if ctx.channel.category_id in self.data['guild_data'][str(ctx.guild.id)]['denied_categories']:
                    self.debug_print('this category (ID %s, name %s) is blocked on the guild (ID %s, name %s), guild admin block category check failed' % (ctx.channel.category_id, ctx.channel.category.name, ctx.guild.id, ctx.guild.name), 'checks')
        ...
        def check_guild_admin_block_channel(ctx):
            ...
            try:
                ...
                if ctx.channel.id in self.data['guild_data'][str(ctx.guild.id)]['denied_channels']:
                    self.debug_print('this channel (ID %s, name %s) is blocked on the guild (ID %s, name %s), guild admin block channel failed' % (ctx.channel.id, ctx.channel.name, ctx.guild.id, ctx.guild.name), 'checks')
        ...
        async def setnotifychannel(ctx, channel=None, guild=None):
            self.debug_print('setnotifychannel: command used with arguments given: %s %s' % (channel, guild), 'guild_owner_commands')
            _lang = self.getLanguage(ctx.author.id, ctx.guild.id if ctx.guild else None, self.lang)
            ...
            if guild:
                ...
                if not targetguild:
                    ...
                    return
                self.debug_print('setnotifychannel: targetguild specified: ID %s NAME %s' % (targetguild.id, targetguild.name), 'guild_owner_commands')
                targetchannel = self.findchannel(targetguild, channel)
                self.debug_print('setnotifychannel: targetchannel ID %s NAME %s' % (targetchannel.id, targetchannel.name), 'guild_owner_commands')
            elif not ctx.guild:
        ...
        async def debug_aban(ctx, user, date='', *, reason=''):
            _lang = self.getLanguage(ctx.author.id, ctx.guild.id if ctx.guild else None, self.lang)
            targetuser = await self.wrapped_convert(ctx, commands.MemberConverter, commands.UserConverter, user)
            if not targetuser:
                return None
            ...
            if date.lower() != '' or date.lower() != self.lang[_lang]['contents']['formats']['date_relative']['never'].lower() or date.lower() != self.lang[_lang]['contents']['formats']['date_relative']['never']:
        ...
        async def aban(ctx, user, date='', *, reason=''):
            try:
                await debug_aban(ctx, user, date, reason=reason)
            except:
        ...
        async def aadminban(ctx, guild='', date='', *, reason=''):
            ...
            if date.lower() != '' or date.lower() != self.lang[_lang]['contents']['formats']['date_relative']['never'].lower() or date.lower() is not self.lang[_lang]['contents']['formats']['date_relative']['never']:
                date_exp = datetime.datetime.now() + self.parseDuration(langtemplate['contents']['formats']['duration'], date.lower())
        ...
        async def blockchannel(ctx, channel=None, guild=None):
            ...
            if not str(targetguild.id) in self.data['guild_data']:
                self.data['guild_data'][str(targetguild.id)] = {}
            if not 'denied_channels' in self.data['guild_data'][str(targetguild.id)]:
            ...
            #else:
            #    await ctx.send(**self.renderMessage(self.lang[_lang], langtemplate, 'command_errors', self.conf, conftemplate, ''))
            await ctx.send(**self.renderMessage(self.lang[_lang], langtemplate, 'info', self.conf, conftemplate, definition='guild_admin_channel_blocked', definition_required=True, channel=targetchannel.mention, channel_name=targetchannel.name, channel_id=targetchannel.id))
        @commands.command()
        async def blockcategory(ctx, category=None, guild=None):
            ...
            if not str(targetguild.id) in self.data['guild_data']:
                self.data['guild_data'][str(targetguild.id)] = {}
            if not 'denied_categories' in self.data['guild_data'][str(targetguild.id)]:
            ...
            #else:
            #    await ctx.send(**self.renderMessage(self.lang[_lang], langtemplate, 'command_errors', self.conf, conftemplate, ''))
            await ctx.send(**self.renderMessage(self.lang[_lang], langtemplate, 'info', self.conf, conftemplate, definition='guild_admin_category_blocked', definition_required=True, category=targetcategory, category_name=targetcategory.name, category_id=targetcategory.id))
        @commands.command()
        async def unblockchannel(ctx, channel=None, guild=None):
            ...
            if not targetchannel:
                ...
                await ctx.send(**self.renderMessage(self.lang[_lang], langtemplate, 'common_errors', self.conf, conftemplate, 'NotFound', item=channel))
                return
            if not (ctx.author.id == self.owner_id and self.conf['owner_permission_bypass']):
            ...
            if not 'denied_channels' in self.data['guild_data'][str(targetguild.id)]:
                self.data['guild_data'][str(targetguild.id)]['denied_channels'] = []
            if targetchannel.id not in self.data['guild_data'][str(targetguild.id)]['denied_channels']:
                await ctx.send(**self.renderMessage(self.lang[_lang], langtemplate, 'custom_errors', self.conf, conftemplate, definition='no_block_channel', definition_required=True, channel=targetchannel.mention, channel_id=targetchannel.id, channel_name=targetchannel.name))
                return
            else:
                self.data['guild_data'][str(targetguild.id)]['denied_channels'].remove(targetchannel.id)
            #else:
            #    await ctx.send(**self.renderMessage(self.lang[_lang], langtemplate, 'command_errors', self.conf, conftemplate, ''))
            await ctx.send(**self.renderMessage(self.lang[_lang], langtemplate, 'info', self.conf, conftemplate, definition='guild_admin_channel_unblocked', definition_required=True, channel=targetchannel.mention, channel_name=targetchannel.name, channel_id=targetchannel.id))
        @commands.command()
        async def unblockcategory(ctx, category=None, guild=None):
            ...
            if not 'denied_categories' in self.data['guild_data'][str(targetguild.id)]:
                self.data['guild_data'][str(targetguild.id)]['denied_categories'] = []
            if targetcategory.id not in self.data['guild_data'][str(targetguild.id)]['denied_categories']:
                await ctx.send(**self.renderMessage(self.lang[_lang], langtemplate, 'custom_errors', self.conf, conftemplate, definition='no_block_category', definition_required=True, category=targetcategory.mention, category_id=targetcategory.id, category_name=targetcategory.name))
                return
            else:
                self.data['guild_data'][str(targetguild.id)]['denied_categories'].remove( targetcategory.id )
            #else:
            #    await ctx.send(**self.renderMessage(self.lang[_lang], langtemplate, 'command_errors', self.conf, conftemplate, ''))
            await ctx.send(**self.renderMessage(self.lang[_lang], langtemplate, 'info', self.conf, conftemplate, definition='guild_admin_category_unblocked', definition_required=True, category=targetcategory, category_name=targetcategory.name, category_id=targetcategory.id))
        @commands.command()
        ...
        async def debug_bans(ctx, guild=None):
            _lang = self.getLanguage(ctx.author.id, ctx.guild.id if ctx.guild else None, self.lang)
            if guild:
                targetguild = await self.wrapped_convert(ctx, GuildConverter, GuildConverter, guild)
                if not targetguild:
            ...
            if not (ctx.author.id == self.owner_id and self.conf['owner_permission_bypass']):
                ...
                self.runtime_check_guild_lpl(targetguild, ctx.author, 2)
            fields = []
            if not 'banned_users' in self.data['guild_data'][str(targetguild.id)].keys():
                self.data['guild_data'][str(targetguild.id)]['banned_users'] = {}
            for each_ban in await targetguild.bans():
                self.data['guild_data'][str(targetguild.id)]['banned_users'][str(each_ban.user.id)] = {'reason': each_ban.reason}
            for userid in self.data['guild_data'][str(targetguild.id)]['banned_users']:
                ...
                if banned: #if user actually exists
                    try:
                        ...
                        date_exp = Never
                    fields.append({'name': {'user_name': banned.name, 'user_discriminator': banned.discriminator, 'user_id': banned.id}, 'value': {'reason': ban['reason'] if 'reason' in ban else None, 'date_exp': date_exp}})
                else:
                    try:
                        date_exp = datetime.datetime(**ban['date_exp'])
                    except (KeyError, AttributeError):
                        date_exp = Never
                    fields.append({'name': {'user_name': ('Unknown User (%s)'%userid), 'user_discriminator': '0000', 'user_id': userid}, 'value': {'reason': ban['reason'] if 'reason' in ban else None, 'date_exp': date_exp}})
        ...
        async def bans(ctx, guild=None):
            try:
                await debug_bans(ctx, guild=None)
            except:
                print( traceback.format_exc() )
    ...
    async def authorban_expiry_timer(self, id):
        async def notify():
            try:
                targetuser = self.get_user(int(id)) or await self.fetch_user(int(id))
                if targetuser:
    ...
    async def authoradminban_expiry_timer(self, id):
        async def notify():
            try:
                targetguild = self.get_guild(int(id)) or await self.fetch_guild(int(id))
    ...
    def formatAverageDuration(self, td, language, shorten=False, in_word=True):
        ...
        return strout
    def renderMessage(self, language, default_language, message_type, config, default_config=None, definition=None, definition_required=False, fields=None, date_relative=False, show_direction=False, duration_options={}, **kwargs):
        """
        ...
        def format_if_str(item, keyitem = ""): #recursive
            try:
                if type(item) is str:
        ...
        def parse():
            ...
            #        #we don't format definition if it is a dictionary
            #        decl = lambda item: format_if_str(_lang_sc['definitions'][definition][item], item) \
            #            if item in _lang_sc['definitions'][definition] \
            #            else (format_if_str(_lang_sc[item], item) if item in _lang_sc else None)
            #    else:
            #        decl = lambda item: format_if_str(_lang_sc[item], item) if item in _lang_sc else None
            #else:
            ...
            self.debug_print('parse called: _lang_sc = \n%s\n_conf_sc = \n%s' % (repr(_lang_sc), repr(_conf_sc)), 'message_rendering')
            def decl(item):
                ...
                if 'definitions' in language["contents"]['messages'][message_type]:
                    ...
                    if definition and (definition in _lang_sc['definitions']) and (item in _lang_sc['definitions'][definition]):
                        self.debug_print('definition is given, definition exists in definitions, item exists in this definition', 'message_rendering')
                        return format_if_str(_lang_sc['definitions'][definition][item], item)
                    elif definition:
                        self.debug_print('definition is given, but neither definition exists nor item is found in it.', 'message_rendering')
                        if (definition not in _lang_sc['definitions']) and definition_required:
                            ...
                            self.debug_print('the problem is - the item doesn\'t exist in definition.', 'message_rendering')
                            ...
                            self.debug_print('definition is not required, meaning that we can replace item by default', 'message_rendering')
                            if definition not in _lang_sc['definitions']:
                                self.debug_print('definition not found, using default item from outside', 'message_rendering')
                                if item in _lang_sc:
                                ...
                                self.debug_print('item is not found on this definition, falling back to the default item given on the root', 'message_rendering')
                                if item in _lang_sc:
                                    self.debug_print('item found in message type', 'message_rendering')
                                    return format_if_str(_lang_sc[item], item)
                                else:
                            ...
                            self.debug_print('definition is required, item and definition probably exists in the dict, then how the heck did we get into there? ', 'message_rendering')
                            return format_if_str(_lang_sc['definitions'][definition][item], item)
                    else:
                        self.debug_print('no definition, probably no item and also no requirement for definition. Returning default item if it exists', 'message_rendering')
                        if item in _lang_sc:
                    ...
                    self.debug_print('there\'s no possible definitions for this message type: %s' % message_type, 'message_rendering')
                    if item in _lang_sc:
    ...
    def getLanguage(self, userid, guildid, languages):
        ...
        if type(userid) not in (int, type(None)):
            self.debug_print('type of given userid is not integer, but %s instead' % type(userid).__name__, 'bot_utility')
            raise ValueError('userid: int or NoneType expected, got %s.' % type(userid).__name__)
        if type(guildid) not in (int, type(None)):
            self.debug_print('type of given userid is not integer, but %s instead' % type(guildid).__name__, 'bot_utility')
            raise ValueError('userid: int or NoneType expected, got %s.' % type(guildid).__name__)
        if (userid == 0 and guildid == 0) or (userid == None and guildid == None):
            ...
            if guildid is not None:
                ...
                if user_override:
                    ...
                    if user_lang not in elangs:
                        self.debug_print('user language is not in present languages, returning default instead', 'bot_utility')
...
def main(args):
    ...
    if 'owner' in conf.keys():
        bot = PtDiscordBot(logger, conf, data, lang, command_prefix=discord.ext.commands.when_mentioned_or(*conf['prefix']), owner_id=int(conf['owner']), intents=intents)
    else:
        bot = PtDiscordBot(logger, conf, data, lang, command_prefix=discord.ext.commands.when_mentioned_or(*conf['prefix']), intents=intents)
    @bot.command()
    @commands.is_owner()
    ...
    async def on_command_error(ctx, exc):
        ...
        if type(exc) not in mod_exc_summary:
            ...
            if type(exc).__name__ in known_exceptions: #                                                                                  v Whoops, we left an exploit
                ...
                await ctx.send(**bot.renderMessage(bot.lang[langcode], langtemplate, msgtype, bot.conf, conftemplate, definition=type(exc).__name__, **kws))
            else:
                await ctx.send(**bot.renderMessage(bot.lang[langcode], langtemplate, 'command_errors', bot.conf, conftemplate, definition=None, exception=str(exc)))
